...
class DataLoader(_DataLoader):
    @classmethod
    def from_datasets(cls, name_or_path: str, split: str, load_on_init: bool = True, constructor: Optional[type] = None, **kwargs):
        if constructor is None:
            constructor = GenericDataset

        if cls._validate_constructor(constructor):
            raise TypeError(f"Construct must be a subclass of "
